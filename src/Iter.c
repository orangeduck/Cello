...
static var Zip_Subtype(var self) {
  return Tuple;
}

static size_t Zip_Len(var self) {
  /* TODO: Check for empty zip */
  return len(get(z->iters, $I(0)));
}
...
var Zip = Cello(Zip,
  ...
  Instance(New,      Zip_New, NULL),
  Instance(Subtype,  Zip_Subtype),
  Instance(Len,      Zip_Len),
  Instance(Get,      Zip_Get, NULL, Zip_Mem, NULL),
  Instance(Iter, 
    Zip_Iter_Init,   Zip_Iter_Next, 
    Zip_Iter_Last,   Zip_Iter_Prev),
  Instance(Show,     Zip_Show, NULL));
